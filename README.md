...
"AI 활용 제안 받기"와 "강의계획서 문구 생성" 버튼이 동작하려면 Anthropic API 키가 필요합니다.

1. https://console.anthropic.com 에서 API 키를 발급받습니다 (사용량에 따라 과금됩니다)
2. Vercel 프로젝트 → **Settings → Environment Variables**
3. Key: `ANTHROPIC_API_KEY`, Value: 발급받은 키 → **Save**
...
여러 학과장이 공유하는 실시간 저장소가 필요하시면, Vercel Postgres/KV 같은 데이터베이스를 붙이거나, 지금처럼 Claude 아티팩트 버전을 함께 쓰시는 것을 추천드립니다 — 그 버전은 모든 방문자가 같은 데이터를 실시간으로 보고 편집할 수 있습니다.
